...
next
  ...
  then have 2:\<open>\<not> member a (remove (B @ A) B)\<close> 
    by (metis Cons.prems member.simps(1) common.simps(2) removent2 set_equal_def)
  ...
  moreover have \<open>set_equal ... (remove (a # B @ A) (a # B))\<close> 
    by (metis member.simps(2) remove.simps(2) union_with_equal union.simps(1) unionaddnt)
...
lemma \<open>set_equal (add f (add n (LA U RA U RN U LP U RP U R U (p U Q))))
  (add f (LA U RA U RN U LP U RP U R U  add n (p U Q)))\<close> 
